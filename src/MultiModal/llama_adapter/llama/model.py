...
class Attention(torch.nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter: torch.Tensor = None,
    ):
        bsz, sql, _ = x.shape
        ...
        if adapter is not None:
            ...
            adapter_k = self.wk(adapter).view(bsz, adapter_len, self.n_heads, self.head_dim)
            adapter_v = self.wv(adapter).view(bsz, adapter_len, self.n_heads, self.head_dim)

...
def load_data():
    ...
    x_train, y_train, x_test, y_test = pickle.load(open("/Users/a1/PythonProgram/zmcode/src/RL/ClassHomework/first experiment/mnist.pkl", 'rb'))
...
class Cnn_net(nn.Module):
    ...
    def forward(self, x):
        x = self.dropout(self.pooling1(self.layer1(x)))
        x = self.dropout(self.pooling2(self.layer2(x)))
...
def train(x_train, y_train, x_test, y_test, BATCH_SIZE, model, loss_function, optimizer):
    ...
    print("毛子鸣's procedure has been completed")#将XXX改为你的中文名字，最终提交运行截图

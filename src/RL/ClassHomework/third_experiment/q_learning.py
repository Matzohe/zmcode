...
def Qlearning(current_state, next_state, action):
    global Q, alpha, gamma, reward
    Q[current_state][action] += alpha * (reward[next_state[0], next_state[1]] + gamma * (max(Q[next_state]) - Q[current_state][action]))
...
def step():
    ...
    global current_pos, epsilon_delta, epsilon, n
    ...
    if new_pos not in terminals:
        Qlearning(current_pos, new_pos, action)
